...
def fetch_mars_rover_photos(arguments: Dict[str, Any])-> Any:
    ...
    params = {
        'api_key': api_key,
        'camera': arguments.get('camera', 'CHEMCAM'), # 默认为 CHEMCAM 相机
        'page': arguments.get('page', 1) # 默认为第一页
    }
    
    if arguments.get('query_type') == 'sol':
        params['sol'] = arguments.get('sol', 1000) # 默认为第1000天
    elif arguments.get('query_type') == 'date':
        params['earth_date'] = arguments.get('earth_date', '2015-06-03') # 默认为2015年6月3日
...
tools = [
    ...
    # 定义第二个工具, 用于获取火星车在火星上拍摄的照片
    {
        ...
        "function":     
        {
            ...
            "parameters": {
                "type": "object",
                "required": ["query_type", "sol", "earth_date", "camera", "page"],
                "properties": {
                    "query_type": {
                        "type": "string",
                        "description": """
